...
**嚴格按照以下樣板輸出，不要增減 section、不要改變 emoji 標題、不要加開場白或結語。**
...
• 版本: [version]
• 運行時間: [X 天 Y 小時]
• 連線: [current]/[current+available] ([百分比]%)
• 磁碟: [shard1] [used_pct]% ([free_gb] GB free) | [shard2] ...
• Chunk 分佈: [均衡/不均衡，簡述]
• Balancer: [狀態] [activeWindow 若有]
• 節點: [全部正常 / X 個異常]
• 平均寫入速率: [insert/uptime_seconds] ops/s

📋 建議行動
• 無
（或逐條列出，每條標注 [可立即執行] / [需協調] / [持續觀察]，含可執行的 MongoDB 指令）
```

### 格式規則
- 每個 section 用 emoji 標題開頭，**不要用 # 或 ### markdown 標題**
- 列表用 `• ` 開頭（bullet point），不要用 `- ` 或數字編號
- 🟢 健康摘要的欄位**全部都要列出**，不可省略
- 問題描述要包含具體數值，不要只說「偏高」
- 如果某 section 沒有內容，寫 `• 無`
